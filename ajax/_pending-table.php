...
//@@@@@@@@@@@@@@@@@@@@@@@  TOTOAL RECORDS @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
$sql = "SELECT * FROM requisition, sysuser WHERE req_app1_user_id IS NOT NULL AND req_pushed IS NULL AND req_status = 1 AND user_id=req_added_by AND req_added_by IS NOT NULL $search";
$db = new Db();
...
if($searchWord !== '' && $searchWord !== '0'){
	$sql = "SELECT * FROM requisition, sysuser WHERE req_app1_user_id IS NOT NULL AND req_pushed IS NULL AND req_status = 1 AND user_id=req_added_by AND req_added_by IS NOT NULL $search $orderby $limit";
	$db = new Db();
	$select = $db->select($sql);
...
echo '<table class="code-eagle-table" cellspacing="0" cellpadding="2" border="1" width="100%" id="table">';
	...
	foreach($select as $row){
		...
		echo '<span id="btn'.$req_id.'" style="margin:5px;" class="push-to-sun btn btn-xs btn-success" data-id="'.$req_id.'" data-req="'.$req_number.'">Push to SUN</span></td>';
		echo '</tr>';
        echo '<tr class="req'.$req_id.'">';
...
<script type="text/javascript">
    $(function(){
        ...
        $(document).off('click','.push-to-sun').on('click', '.push-to-sun', function(e){
