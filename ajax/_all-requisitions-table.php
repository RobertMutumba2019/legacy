...
//@@@@@@@@@@@@@@@@@@@@@@@  TOTOAL RECORDS @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
$sql = "SELECT * FROM requisition, sysuser WHERE user_id=req_added_by AND req_added_by IS NOT NULL $search";
$db = new Db();
...
if($searchWord !== '' && $searchWord !== '0'){
	$sql = "SELECT * FROM requisition, sysuser WHERE user_id=req_added_by AND req_added_by IS NOT NULL $search $orderby $limit";
	$db = new Db();
	$select = $db->select($sql);
