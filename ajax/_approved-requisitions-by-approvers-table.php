...
$sql = "SELECT * FROM requisition WHERE req_app1_date IS NOT NULL AND req_division = '$div' $search";
$db = new Db();
...
if($searchWord !== '' && $searchWord !== '0'){
	$sql = "SELECT * FROM requisition WHERE req_app1_date IS NOT NULL AND req_division = '$div' $search $orderby $limit";
	$db = new Db();
	$select = $db->select($sql);
