...
//@@@@@@@@@@@@@@@@@@@@@@@  TOTOAL RECORDS @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
$sql = "SELECT * FROM requisition WHERE req_app1_date IS NULL AND req_added_by = '$user_id' AND req_status = '-1' $search";
$db = new Db();
...
if($searchWord !== '' && $searchWord !== '0'){
	$sql = "SELECT * FROM requisition WHERE req_app1_date IS NULL AND req_added_by = '$user_id' AND req_status = '-1' $search $orderby $limit";
	$db = new Db();
	$select = $db->select($sql);
