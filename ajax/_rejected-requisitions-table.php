...
//@@@@@@@@@@@@@@@@@@@@@@@  TOTOAL RECORDS @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
$sql = "SELECT * FROM requisition WHERE req_status = '0' AND req_added_by = '$user_id' $search";
$db = new Db();
...
if($searchWord !== '' && $searchWord !== '0'){
	$sql = "SELECT * FROM requisition WHERE req_status = '0' AND req_added_by = '$user_id' $search $orderby $limit";
	$db = new Db();
	$select = $db->select($sql);
