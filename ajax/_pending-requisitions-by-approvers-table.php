...
if($searchWord !== '' && $searchWord !== '0'){
	$sql = "SELECT * FROM requisition WHERE  req_status = 1 AND req_app1_date IS NULL AND req_division = '$div' $search $orderby $limit";
	$db = new Db();
	$select = $db->select($sql);
...
//echo $sql;
	...
	if(is_array($select)){
		//ensures that it only enters the loop when the variable is accessible/iterable
