...
class PHPMailer
{
    ...
    public function punyencodeAddress($address)
    {
        ...
        $pos = strrpos($address, '@');
        if (static::idnSupported() && !empty($this->CharSet) && false !== $pos
        ) {
    ...
    public function preSend()
    {
        ...
        if (ini_get('mail.add_x_header') == 1 && 'mail' == $this->Mailer && stripos(PHP_OS, 'WIN') === 0 && (version_compare(PHP_VERSION, '7.0.0', '>=') && version_compare(PHP_VERSION, '7.0.17', '<') || version_compare(PHP_VERSION, '7.1.0', '>=') && version_compare(PHP_VERSION, '7.1.3', '<'))
        ) {
            trigger_error(
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if (!empty($this->Sender) && self::isShellSafe($this->Sender)) {
            $sendmailFmt = 'qmail' == $this->Mailer ? '%s -f%s' : '%s -oi -f%s -t';
    ...
    public function smtpConnect($options = null)
    {
        ...
        if ($this->exceptions && $lastexception instanceof \Exception) {
            throw $lastexception;
        }
    ...
    public function msgHTML($message, $basedir = '', $advanced = false)
    {
        ...
        foreach ($images[2] as $imgindex => $url) {
            ...
            if (// Only process relative URLs if a basedir is provided (i.e. no absolute local paths)
                !empty($basedir) && strpos($url, '..') === false && 0 !== strpos($url, 'cid:') && !preg_match('#^[a-z][a-z0-9+.-]*:?//#i', $url)
            ) {
